...
class Gambling(commands.Cog):
  ...
  async def coinflip(self, ctx, choice: str, bet: int):
    ...
    if ban_data is None:
      if bet < 10:
        await ctx.reply(f"🧏‍♀️ You need to bet at least 10 <:coin:1178687013583585343>")
        ...
        if bet > coins:
          await ctx.reply(
            f"🧏‍♀️ really?, you are broke you only have {coins} <:coin:1178687013583585343>")
          return
  ...
  async def rps(self, ctx, member: discord.Member, bet: int):
    ...
    if bet < 10:
      embed = discord.Embed(
        description=f"🧏‍♀️ You need to bet at least 10 <:coin:1178687013583585343>",
        color=0xF2A2C0)
      await ctx.send(embed=embed)
    elif bet > author_coin:
      embed = discord.Embed(
        description=f"{ctx.author.mention} really?, 🧏‍♀️ You only have {author_coin} <:coin:1178687013583585343>",
        color=0xF2A2C0)
      await ctx.send(embed=embed)
    elif bet > member_coin:
      embed = discord.Embed(
        description=f"🧏‍♀️, {member.mention} only have {member_coin} <:coin:1178687013583585343>",
        color=0xF2A2C0)
      await ctx.send(embed=embed)
    else:
      await ctx.send(
